...
std::vector<size_t> AcsFindFuzzyMatches(const std::string& pattern_with_wildcards, const std::string& text, const char wildcard) {
    ...
    for (size_t text_index = 0; text_index != text.size(); ++text_index) {
        wildcard_matcher.Scan(text[text_index], [&fuzzy_matches_begin_indices, text_index, pattern_size]() {fuzzy_matches_begin_indices.push_back(text_index + 1 - pattern_size); });
    }

    return fuzzy_matches_begin_indices;
}

std::vector<size_t> AcsFindFuzzyMatchesFromFile(const std::string& pattern_with_wildcards, const std::string& file, const char wildcard) {
    return AcsFindFuzzyMatches(pattern_with_wildcards, readfile(file), wildcard);
}

std::vector<size_t> AcsFindFuzzyHexMatches(std::string& pattern_with_wildcards, const std::string& text, const char wildcard) {
    pattern_with_wildcards.erase(std::remove_if(pattern_with_wildcards.begin(), pattern_with_wildcards.end(), isspace), pattern_with_wildcards.end());
    ...
    std::vector<size_t> fuzzy_matches_begin_indices;
    for (size_t text_index = 0; text_index != text.size(); ++text_index) {
        wildcard_matcher.Scan(text[text_index], [&fuzzy_matches_begin_indices, text_index, pattern_size]() {fuzzy_matches_begin_indices.push_back((text_index + 1 - pattern_size)/2); });
    }

    return fuzzy_matches_begin_indices;
}

std::vector<size_t> AcsFindFuzzyHexMatchesFromFile(std::string& pattern_with_wildcards, const std::string& file, const char wildcard) {
    return AcsFindFuzzyHexMatches(pattern_with_wildcards, readfile(file, true), wildcard);
}
